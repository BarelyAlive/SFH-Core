...
public class GuiHandler implements IGuiHandler {
	...
	protected static List<Pair<TileEntity, Container>> teco = new ArrayList<Pair<TileEntity, Container>>();
	...
	public Object getServerGuiElement(int ID, EntityPlayer player, World world, int x, int y, int z) {
		TileEntity te1 = world.getTileEntity(new BlockPos(x, y, z));
			...
			for(Pair<TileEntity, Container> tc : teco) {
				if(te1.getClass().equals(tc.getLeft().getClass())){
					
					Class<Container> c = (Class<Container>) tc.getRight().getClass();
					try {
	 ...
	 * @return
	 */
	public static boolean addGUIRelation(TileEntity tile, Container con) {
		return teco.add(new ImmutablePair<TileEntity, Container>(tile, con));
	}
}
